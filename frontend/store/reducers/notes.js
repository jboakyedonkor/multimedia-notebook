import { SET_NOTES, UPDATE_NOTE_WITH_NEW_TITLE, SET_SEARCHED_NOTES, SET_FAVORITE_NOTES, CREATE_NOTE, UPDATE_NOTE, DELETE_NOTE, ADD_TO_FAVORITE, REMOVE_FROM_FAVORITE } from '../actions/notes';
...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case UPDATE_NOTE:
      ...
      case UPDATE_NOTE_WITH_NEW_TITLE:
        const currentNoteIndex = state.allNotes.findIndex(
          note => note.name === action.noteData.old_name
        );
  
        const updatedTitleNote = new Note(
          action.noteData.name,
          action.noteData.text,
          action.noteData.video_link,
          action.noteData.audio_link,
        ...
        const updatedTitleNotes = [...state.allNotes];
        updatedTitleNotes[currentNoteIndex] = updatedTitleNote;
  
        //next check to see if searched note needs to be updated
        const index = state.searchedNotes.findIndex(
          note => note.name === action.noteData.old_name
        );
        const updatedTitleSearchedNotes = [...state.searchedNotes];
        if (index >= 0) {
          updatedTitleSearchedNotes[index] = updatedTitleNote;
        }
  
        return {
          ...state,
          allNotes: updatedTitleNotes,
          searchedNotes: updatedTitleSearchedNotes
        };
